...
class EsamenesScreen(Screen):
    ...
    def _popup_nuevo(self):
        ...
        btn_guardar = Factory.BotonAccion(text="Generar claves", size_hint_y=None, height=dp(48))
        cont.add_widget(btn_guardar)
        
        pop = Popup(title="Crear examen", content=cont, size_hint=(0.95, 0.75))
        ...
        def _guardar(*_):
            ...
            app.temp_examen_nombre = nombre
            app.temp_examen_cantidad = cantidad
            app.temp_examen_pdf = chk.active
            
            pop.dismiss()
            app.sm.current = "generador_claves"
...
# ─────────────────────────────────────────────────────────────────────────────
# PANTALLA: GENERADOR DE CLAVES (Paso 3.1)
# ─────────────────────────────────────────────────────────────────────────────
class GeneradorClavesScreen(Screen):
    def __init__(self, **kw):
        super().__init__(**kw)
        self.cont = BoxLayout(orientation="vertical", padding=dp(14), spacing=dp(8))
        self.add_widget(self.cont)
    ...
    def construir(self):
        app = App.get_running_app()
        nombre = getattr(app, "temp_examen_nombre", "Desconocido")
        cantidad = getattr(app, "temp_examen_cantidad", 10)

        self.cont.clear_widgets()
        self.spinners = []

        self.cont.add_widget(Encabezado(titulo="Generando claves", subtitulo=f"Examen: {nombre}"))

        scroll = ScrollView()
        grid_cols = BoxLayout(orientation="horizontal", spacing=dp(15), size_hint_y=None)
        grid_cols.bind(minimum_height=grid_cols.setter('height'))

        col_izq = BoxLayout(orientation="vertical", spacing=dp(8), size_hint_y=None)
        col_izq.bind(minimum_height=col_izq.setter('height'))
        ...
        if cantidad <= 10:
            ...
            limite_izq = (cantidad + 1) // 2 
            col_der = BoxLayout(orientation="vertical", spacing=dp(8), size_hint_y=None)
            col_der.bind(minimum_height=col_der.setter('height'))

        for i in range(1, cantidad + 1):
            fila = BoxLayout(size_hint_y=None, height=dp(44), spacing=dp(8))
            
            lbl_num = Label(text=f"{i}.", size_hint_x=0.3, halign="right", valign="middle")
            lbl_num.bind(size=lambda inst, *_: setattr(inst, "text_size", (inst.width, inst.height)))
            fila.add_widget(lbl_num)

            sp = Spinner(text="A", values=["A", "B", "C", "D"], size_hint_x=0.7)
            self.spinners.append(sp)
            fila.add_widget(sp)

            if i <= limite_izq:
                col_izq.add_widget(fila)
            else:
                col_der.add_widget(fila)

        grid_cols.add_widget(col_izq)
        if col_der:
            grid_cols.add_widget(col_der)

        scroll.add_widget(grid_cols)
        self.cont.add_widget(scroll)

        btn_guardar = Factory.BotonAccion(text="Finalizar y guardar", size_hint_y=None, height=dp(52))
        btn_guardar.bind(on_release=self._guardar)
        self.cont.add_widget(btn_guardar)

        btn_volver = Button(text="Cancelar", size_hint_y=None, height=dp(44))
        btn_volver.bind(on_release=lambda *_: setattr(app.sm, "current", "examenes"))
        self.cont.add_widget(btn_volver)

    def _guardar(self, *_):
        app = App.get_running_app()
        nombre = app.temp_examen_nombre
        cantidad = app.temp_examen_cantidad
        pdf_chk = app.temp_examen_pdf

        clave_letras = [sp.text for sp in self.spinners]

        guardar_clave(app.ruta_curso, nombre, cantidad, clave_letras)
...
class CalificadorApp(App):
    ...
    def build(self):
        ...
        self.sm.add_widget(GeneradorClavesScreen(name="generador_claves"))
